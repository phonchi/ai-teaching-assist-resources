...
- agent 改壞了，怎麼回到上一版？
- 老師與 agent 同時改，怎麼避免互相覆蓋？
- 網站或講義出問題，怎麼知道是哪一次修改造成的？
- 一學期改了很多次，怎麼留下教材修改日誌？

Git 就是教材的「存檔點」與「時光機」；GitHub 則讓這份修改歷史可以被老師與 agent 共同使用。
...
| GitHub | <https://github.com/> | 雲端協作：老師與 agent 共用同一份修改歷史 |
...
| 投影片用語 | 白話意思 | 教學情境 |
|---|---|---|
| 可協作 | 多人接力，不互相覆蓋 | 老師與 agent 都能在同一份教材上工作 |
...
在開始交給 agent 修改前，先存一版。新手優先用 GitHub Desktop 的 `Commit` 按鈕；熟悉 terminal 後也可以用：
...
Agent 改完後，不要直接上傳。新手先看 GitHub Desktop 的 `Changes` 畫面；熟悉 terminal 後也可以看：
...
確認內容合理後再 commit。新手用 GitHub Desktop；熟悉 terminal 後也可以用：
...
新手可用 GitHub Desktop 的 `Push origin`；熟悉 terminal 後也可以用 `git push`。

這一步不是單純「放上 GitHub」，而是把這次教材修改同步到共同歷史，方便之後追查與網站部署。

## 如果 agent 改壞了怎麼辦？

先看有哪些檔案被改。新手看 GitHub Desktop 的 `Changes`；熟悉 terminal 後也可以用：
...
如果還沒 commit，可以請主 agent 根據差異摘要修正，或人工只保留好的部分。若已經 commit，先不要慌，Git 的重點就是每個存檔點都還在。新手建議先不要自行刪歷史，先用 GitHub Desktop 看歷史，再決定是否回復到指定 commit。
