...
> 進階提醒：Skill 不是第一天要做的事。先熟悉網頁版 AI、GitHub Desktop 存檔點、Codex app / Claude Code desktop app 的只讀與小修改流程，再來封裝 Skill。

投影片中的說法是：把跑順的流程變成「實驗室手冊」。以後遇到同類任務，agent 不必每次重新理解你的偏好，而是直接套用這份手冊。

## 什麼時候該寫 Skill？

不要第一天就寫 Skill。先用單次 prompt 與桌面版 agent 跑幾次安全任務，再用 `CLAUDE.md` / `AGENTS.md` 固定全域規矩，最後才把反覆摩擦的流程封裝成 Skill。適合封裝的時機：
